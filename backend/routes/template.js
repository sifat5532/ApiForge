...
router.post('/like', requireAuth, templateExistence, async (req, res) => {
    ...
    if(!template_id || !isLike){
        return res.status(400).json({ msg: "You should input template_id and isLike"});
    }
    ...
    if (!isLike) return res.status(400).json({ msg: "You have not liked the template yet" });
