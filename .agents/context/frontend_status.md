...
| Projects — shared card variant | `.project-card--shared`, `.project-card__author-row` |
...
| `loadProjectHeader()` | `GET /view/viewProject/:id` → renders title, clone badge, meta chips, tags; stores `authorId`; computes `isAuthor`; applies all author-only button visibility (regen key, leave, invite, add FK, add CORS, create table) |
...
| `loadApis()` | `GET /view/apis/:id` → renders professional API cards (method pill, name, active status, public call URL with copy, rate limit, created date) |
| `apiCardHtml(api)` / `buildApiUrl(api)` | Build one API card; `buildApiUrl` composes `BACKEND/api/:author_username/:project_name/:api_name` |
| `copyApiUrl(btn)` / `editApi(id)` / `deleteApi(id, name)` | Copy call URL to clipboard; edit (defers to future API builder); delete via `POST /project/deleteApi` |
| `openApiDetailsModal(id)` | "View details" button → modal listing every dynamic param (name, source, required, fallback) parsed from `query_definition` + example request |
| `extractApiParams(method, def)` / `collectWhereParams(nodes, out)` / `paramFromWhereVal(val, ctx)` / `paramFromValueObj(v)` / `normaliseSource(raw)` | Walk a stored `query_definition` (GET where/having recursion, POST/PUT value_obj_array, PUT/DELETE where) and return caller-supplied dynamic inputs |
| `buildExampleCall(...)` / `buildExampleBody(bodyParams)` | Compose an example call URL / JSON body using fallback values |
| `openAddApiModal()` | `+ API` button → modal explaining endpoints are created in the (future) API builder |
...
| `bindGlobalActions()` | Wires Add FK / Add CORS / Invite / Delete / Regen Key buttons; visibility is driven by `loadProjectHeader()` |
...
- **View Project author vs collaborator UI (2026-09-04)**: The `view-project.html` / `view-project.js` page strictly enforces role-based UI. All role gating is driven by `vpState.isAuthor` (set in `loadProjectHeader()` after comparing `loggedInUserId` to `project.author_id`). The following rules MUST be preserved on any future page generation:

  | UI Element | Author | Collaborator |
  |---|---|---|
  | "Regenerate API Key" button (`#vp-regen-key`) | Visible | Hidden |
  | "Leave project" button (`#vp-leave-project`) | Hidden | Visible |
  | "+ Invite" button (`#btn-invite-collab`) | Visible | Hidden |
  | "+ Add FK" button (`#btn-add-fk`) | Visible | Hidden |
  | "+ Add Origin" (CORS) button (`#btn-add-cors`) | Visible | Hidden |
  | "+ Create Table" button (`#btn-create-table`) | Visible | Hidden |
  | FK row "Remove" buttons | Visible | Hidden |
  | Collaborator "Remove" buttons (others) | Visible | Hidden |
  | Collaborator own "Leave" button | Visible | Visible (self only) |
  | Pending invite "Cancel" button | Visible | Hidden |
  | Settings form inputs / Save / Danger zone | Editable + visible | Disabled + hidden |
  | Settings read-only banner | Hidden | Visible |

  **Implementation pattern**: all button visibility is applied in `loadProjectHeader()` immediately after `vpState.isAuthor` is known, so it takes effect before any tab is lazy-loaded. Tab-load functions (`loadTables`, `loadForeignKeys`, `loadCorsOrigins`, `loadCollaborators`) apply the same gating a second time as a defensive guard. The Collaborators `/view/collaborators/:projectId` route returns HTTP 404 when there are no collaborators — the frontend treats 404 as an empty list (not an error).

- **APIs tab built (2026-09-04)**: The APIs tab in `view-project.html` was converted from a "Coming soon" stub to a live list. `loadApis()` fetches `GET /view/apis/:projectId` and renders professional API cards showing: HTTP method pill (`.vp-api-method--*`), API name, Active/Inactive status, the **public call URL** (`BACKEND/api/:author_username/:project_name/:api_name`) with a one-click **Copy** button, plus rate-limit and created-date meta. Each card has **View details**, **Edit** and **Delete** actions; a **+ API** button lives in the panel action bar. Backend changes were required and made (user-approved): (1) `GET /view/apis/:projectId` had a bug — it queried with `req.params.tableId` (undefined) instead of `req.params.projectId`; fixed. It now also JOINs `users`/`projects` to return `author_username` and `project_name` so the frontend can build the public URL. (2) Added `POST /project/deleteApi` (`{ proj_id, api_id }`), gated to the project author or an accepted `editor` collaborator, matching the `removeForeignKey` permission pattern. **"View details" modal**: `openApiDetailsModal()` parses the stored `query_definition` (the JSONB produced by `backend/routes/api.js`: GET `where`/`having` with recursive `group`/`children`, POST/PUT `value_obj_array`, PUT/DELETE `where`) via `extractApiParams()` and renders a parameter table per source — Route param / Query parameter / Body field — showing field name, Required/Optional, fallback/default value, and where it is used. It also shows auth requirement (`x-api-key` header when `auth_enabled`), rate limit, GET paging (`limit`/`offset`/`allow_client_paging`), and a copy-ready example request. **Deferred**: creating/editing an API requires the visual query builder (`query_definition` JSONB built from the project catalog via `POST /new/api/create`) which is out of scope for this view — the `+ API` and per-row **Edit** actions currently surface a "coming soon" message pointing to the API builder rather than embedding a full builder here.

- **Create Table modal added (2026-09-04)**: `openCreateTableModal()` / `ctAddColumn()` / `submitCreateTable()` added to `js/view-project.js`. The modal uses `vp-modal--wide` class on the overlay for extra width. Columns are built as dynamic cards with: column name, type select, optional length (shown only for `VARCHAR`/`NUMERIC`), Primary Key / Auto Increment / Unique / Nullable checkboxes, and a Default value input. Logic enforced in the frontend: (1) PK → Unique and Nullable are disabled and forced off; (2) Auto Increment → type must be `INTEGER` and column must be PK or Unique; (3) VARCHAR/NUMERIC → length is required and must be ≥ 1. Submits `POST /project/createTable` with `{ proj_id, name, cols }` where each col is `[col_name, col_type, default, col_len, is_pk, is_auto_inc, is_nullable, is_unique, element_id]`. Backend per-column errors (returned with `id`) are highlighted on the matching card. On success, the Tables tab reloads. The `+ Create Table` button in `view-project.html` was converted from `<a>` to `<button>` and wired via `bindGlobalActions()`. `closeModal()` now always strips the `vp-modal--wide` class. New CSS classes added to `style.css`: `.vp-modal--wide`, `.ct-name-row`, `.ct-col-list`, `.ct-col-card`, `.ct-col-card--error`, `.ct-col-top`, `.ct-col-len-wrap`, `.ct-col-len-wrap.ct-hidden`, `.ct-col-bottom`, `.ct-check`, `.ct-check.ct-disabled`, `.ct-col-default`, `.ct-col-delete`, `.ct-add-col-row`, `.ct-col-error`, `.ct-col-error.is-visible`, `.ct-form-error`, `.ct-form-error.is-visible`.
